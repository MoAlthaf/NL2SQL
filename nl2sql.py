...
def extract_schema(prompt: str) -> str:
    """
    Extracts and cleans the schema from a given prompt string.
    Returns the schema string without the final pipe-delimited element.
    """
    try:
        schema = prompt.split("Convert text to NatSQL:")[1].split("|", 1)[1]
        return "|".join(schema.split("|")[:-1])
    except (IndexError, AttributeError):
        return ""

def build_prompt(question: str, original_prompt: str) -> str:
    """Builds a prompt for the LLM using the question and schema."""
    schema = extract_schema(original_prompt)
    return (
        f"Convert the text to MySQL query,Follow the system insturction strictly. Here is the Question: {question}.\n"
        f"Here is database schema for context: {schema}\n\n"
    )

def generate_sql(prompt: str,question: str) -> str:
...
def reverifier(prompt: str, initial_sql: str, db_id: str, gt_sql: str,question: str, max_retries: int = 5) -> tuple[str, bool, int]:
    """
    ...
    while not matched and attempts <= max_retries:
        ...
        print("Reverifier attempt:", attempts)
...
if __name__ == "__main__":
    ...
    for i, row in new_df.iterrows():
        ...
        question = row.get('Question', '') #Change it to 'paraphrased_nl'
        original_prompt = row.get('prompt', '')
        gt_sql = row.get('gt_nl', '')
        db_id = row.get('db_id', '')

        #prompt = build_prompt(question, original_prompt)

        # Generate SQL
        try:
            generated_sql = generate_sql(original_prompt,question)
        except Exception as e:
        ...
        try:
            ...
            if matched:
                ...
                generated_sql, matched, reverifier_attempts = reverifier(original_prompt, generated_sql, db_id, gt_sql,question=question)   # Debugging reverifier
    ...
    new_df[["db_id", "original_nl", "paraphrased_nl", "gt_nl", "generated_sql", "matched", "attempts"]].to_csv("result/results.csv", index=False)
